...
type CursorConfig = typeof MEDUSAE_DEFAULTS.cursor;
type HaloConfig = typeof MEDUSAE_DEFAULTS.halo;
type ParticleConfig = typeof MEDUSAE_DEFAULTS.particles;
type BackgroundConfig = typeof MEDUSAE_DEFAULTS.background;
...
type MedusaeConfigOverrides = Partial<{
...
const mergeConfig = (config?: MedusaeConfigOverrides): MedusaeConfig => ({
...
const Particles = ({ config }: { config?: MedusaeConfigOverrides }) => {
  ...
  const baseWidth = typeof window !== 'undefined' ? window.innerWidth : 1;
  const baseHeight = typeof window !== 'undefined' ? window.innerHeight : 1;
  ...
  const uniforms = useMemo(
    () => ({
      ...
      uResolution: {
        value: new THREE.Vector2(baseWidth, baseHeight),
      },
  ...
  useFrame((state) => {
    ...
    if (hovering.current) {
      const pointerSource =
        globalPointer.current ?? ({ x: pointer.x, y: pointer.y } as const);
...
export const MedusaeBackdrop: React.FC<MedusaeBackdropProps> = ({
  ...
  const themeConfig = useMemo<MedusaeConfigOverrides>(
    () => ({
      background: {
        color: isDark ? '#06080d' : '#f8fafc',
      },
      particles: {
        colorBase: isDark ? '#05070c' : '#e2e8f0',
        colorOne: isDark ? '#22d3ee' : '#0ea5e9',
        colorTwo: isDark ? '#38bdf8' : '#14b8a6',
        colorThree: isDark ? '#f59e0b' : '#f59e0b',
      },
    }),
    [isDark],
  );
